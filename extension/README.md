...
```bash
mbridge bridge install --extension-id <粘贴刚才的ID>
```

这会写入宿主 manifest + 启动器，并在注册表（Windows）/ 配置目录（mac/Linux）
注册 Chrome 和 Edge。只想注册 Chrome 加 `--chrome-only`。
...
- 帧协议 / 宿主 / 安装器：`modelbridge/bridge/`（`protocol.py` `host.py`
  `session_runner.py` `browser_bridge.py` `install.py`）。
- 远程浏览器工具：`modelbridge/agent/tools/browser_tools.py` +
  `browser_write_tools.py`，DOM 执行逻辑在本目录 `sidepanel.js` 的
  `pageToolDispatcher`（注入到页面运行）。
- 测试：`tests/test_bridge_*.py`（协议、bridge、runner、host 全闭环，无需浏览器）。
- 手动冒烟：`mbridge bridge run` 然后用脚本喂帧（见 `tests/test_bridge_protocol.py`）。
