...
PROFILES: dict[ProviderType, ProviderProfile] = {
    ...
    ProviderType.KIMI: ProviderProfile(
        ...
        default_capabilities=_CLOUD_CAPS,
        notes="OpenAI 端点 https://api.moonshot.ai/v1；Anthropic 兼容端点 /anthropic。K3 为旗舰(1M ctx,默认 reasoning)；K2.6 thinking 模型返回 reasoning_content；temperature 通常应=0。",
    ),
    ...
    ProviderType.MINIMAX: ProviderProfile(
        ...
        default_level=ModelLevel.AGENT,
        default_capabilities=_CLOUD_CAPS,
        notes="OpenAI 兼容 https://api.minimaxi.com/v1；Anthropic 兼容 /anthropic。国内 minimaxi.com / 国际 minimax.io，账号互不通用。model id 官方写法首字母大写含点(MiniMax-M3)；M3 当前 50% 促销。",
    ),
